...
def upgrade() -> None:
    ...
    op.create_table(
        'product_families',
        sa.Column('family_id', sa.Integer(), autoincrement=True, nullable=False, comment='Primary key (auto-increment)'),
        sa.Column('category_id', sa.Integer(), nullable=False, comment='Foreign key to product_categories (CASCADE delete)'),
        sa.Column('name', sa.String(length=200), nullable=False, comment='Human-readable family name (e.g., Echeveria, Aloe)'),
    ...
    # Create indexes
    op.create_index(op.f('ix_product_families_category_id'), 'product_families', ['category_id'], unique=False)
    ...
    op.execute("""
        ...
        FROM product_categories WHERE code = 'FERN';
    """)
    ...
    op.execute("""
        ...
        FROM product_categories WHERE code = 'TROPICAL';
    """)
    ...
    op.execute("""
        ...
        FROM product_categories WHERE code = 'HERB';
    """)


def downgrade() -> None:
    """Drop product_families table."""
    op.drop_index(op.f('ix_product_families_category_id'), table_name='product_families')
