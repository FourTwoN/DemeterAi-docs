...
def upgrade() -> None:
  ...
  op.create_table(
      ...
      sa.Column('id', sa.Integer(), autoincrement=True, nullable=False,
                comment='Primary key'),
      sa.Column('parent_location_id', sa.Integer(), nullable=False,
                comment='Parent location ID'),
      sa.Column('child_location_id', sa.Integer(), nullable=False,
                comment='Child location ID'),
      sa.Column('relationship_type', postgresql.ENUM('contains', 'adjacent_to',
      ...
      sa.CheckConstraint('parent_location_id != child_location_id',
                         name='ck_no_self_reference'),
      sa.ForeignKeyConstraint(['child_location_id'],
                              ['storage_locations.location_id'],
                              ondelete='CASCADE'),
      sa.ForeignKeyConstraint(['parent_location_id'],
                              ['storage_locations.location_id'],
                              ondelete='CASCADE'),
      sa.PrimaryKeyConstraint('id'),
      sa.UniqueConstraint('parent_location_id', 'child_location_id',
                          name='uq_location_pair'),
      comment='Hierarchical and adjacency relationships between storage locations'
  )

  op.create_index('ix_location_relationships_parent_location_id',
                  'location_relationships', ['parent_location_id'])
  op.create_index('ix_location_relationships_child_location_id',
                  'location_relationships', ['child_location_id'])


def downgrade() -> None:
  op.drop_index('ix_location_relationships_child_location_id',
                table_name='location_relationships')
  op.drop_index('ix_location_relationships_parent_location_id',
                table_name='location_relationships')
