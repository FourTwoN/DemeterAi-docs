...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('capacity', sa.Integer(), nullable=True, comment='Total capacity (nullable, may differ from rows×columns)'),
        sa.Column('length_cm', sa.Numeric(precision=6, scale=2), nullable=True, comment='Container length in cm (nullable)'),
        sa.Column('width_cm', sa.Numeric(precision=6, scale=2), nullable=True, comment='Container width in cm (nullable)'),
        sa.Column('height_cm', sa.Numeric(precision=6, scale=2), nullable=True, comment='Container height in cm (nullable)'),
        sa.Column('is_grid', sa.Boolean(), server_default='false', nullable=False, comment='True for plug trays with grid structure (default FALSE)'),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False, comment='Record creation timestamp'),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True, comment='Last update timestamp'),
        sa.CheckConstraint("LENGTH(code) >= 3 AND LENGTH(code) <= 50", name='ck_storage_bin_type_code_length'),
        sa.CheckConstraint("(is_grid = false) OR (is_grid = true AND rows IS NOT NULL AND columns IS NOT NULL)", name='ck_storage_bin_type_grid_requires_rows_columns'),
    ...
    # 3. Create indexes
    op.create_index('ix_storage_bin_types_code', 'storage_bin_types', ['code'], unique=True)
    op.create_index('ix_storage_bin_types_category', 'storage_bin_types', ['category'], unique=False)
