...
def upgrade() -> None:
  ...
  op.create_table(
      'storage_areas',
      sa.Column('storage_area_id', sa.Integer(), autoincrement=True,
      ...
      sa.Column('position',
                postgresql.ENUM('N', 'S', 'E', 'W', 'C', name='position_enum',
      ...
      sa.ForeignKeyConstraint(['warehouse_id'], ['warehouses.warehouse_id'],
                              name='fk_storage_areas_warehouse_id',
                              ondelete='CASCADE'),
      sa.ForeignKeyConstraint(['parent_area_id'],
                              ['storage_areas.storage_area_id'],
                              name='fk_storage_areas_parent_area_id',
                              ondelete='CASCADE'),
  ...
  op.execute("""
             CREATE
             OR REPLACE FUNCTION check_storage_area_within_warehouse()
        RETURNS TRIGGER AS $$
        DECLARE
  ...
  op.execute("""
             CREATE TRIGGER trg_storage_area_containment
                 BEFORE INSERT OR
             UPDATE OF geojson_coordinates, warehouse_id
             ON storage_areas
                 FOR EACH ROW
  ...
  op.execute(
      "CREATE INDEX idx_storage_areas_geom ON storage_areas USING GIST(geojson_coordinates);")
  op.execute(
      "CREATE INDEX idx_storage_areas_centroid ON storage_areas USING GIST(centroid);")

  # Step 9: Create standard B-tree indexes for common queries
  op.create_index('idx_storage_areas_code', 'storage_areas', ['code'],
                  unique=True)
  op.create_index('idx_storage_areas_warehouse_id', 'storage_areas',
                  ['warehouse_id'])
  op.create_index('idx_storage_areas_parent_area_id', 'storage_areas',
                  ['parent_area_id'])
...
def downgrade() -> None:
  ...
  op.execute(
      "DROP TRIGGER IF EXISTS trg_storage_area_containment ON storage_areas;")
  ...
  op.execute(
      "DROP TRIGGER IF EXISTS trg_storage_area_centroid ON storage_areas;")
