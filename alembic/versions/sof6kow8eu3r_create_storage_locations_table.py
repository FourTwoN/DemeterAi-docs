...
def upgrade() -> None:
  ...
  op.create_table(
      ...
      sa.Column('photo_session_id', sa.Integer(), nullable=True,
      ...
      sa.Column('position_metadata', JSONB(), nullable=False,
      ...
      sa.ForeignKeyConstraint(['storage_area_id'],
                              ['storage_areas.storage_area_id'],
                              name='fk_storage_locations_storage_area_id',
                              ondelete='CASCADE'),
      ...
      sa.CheckConstraint('LENGTH(code) >= 2 AND LENGTH(code) <= 50',
                         name='ck_storage_location_code_length'),
      sa.CheckConstraint('LENGTH(qr_code) >= 8 AND LENGTH(qr_code) <= 20',
  ...
  op.execute("""
             CREATE
             OR REPLACE FUNCTION check_storage_location_within_area()
        RETURNS TRIGGER AS $$
        DECLARE
             area_geom geometry;
             BEGIN
            -- Fetch parent storage area geometry (POLYGON)
             SELECT geojson_coordinates
             INTO area_geom
             FROM storage_areas
  ...
  op.execute("""
             CREATE TRIGGER trg_storage_location_containment
                 BEFORE INSERT OR
             UPDATE OF coordinates, storage_area_id
             ON storage_locations
                 FOR EACH ROW
  ...
  op.execute(
      "CREATE INDEX idx_storage_locations_coords ON storage_locations USING GIST(coordinates);")
  op.execute(
      "CREATE INDEX idx_storage_locations_centroid ON storage_locations USING GIST(centroid);")

  # Step 8: Create standard B-tree indexes for common queries
  op.create_index('idx_storage_locations_code', 'storage_locations', ['code'],
                  unique=True)
  op.create_index('idx_storage_locations_qr_code', 'storage_locations',
                  ['qr_code'], unique=True)
  op.create_index('idx_storage_locations_storage_area_id', 'storage_locations',
                  ['storage_area_id'])
  op.create_index('idx_storage_locations_photo_session_id', 'storage_locations',
                  ['photo_session_id'])
  op.create_index('idx_storage_locations_active', 'storage_locations',
                  ['active'])
...
def downgrade() -> None:
  ...
  op.drop_index('idx_storage_locations_photo_session_id',
                table_name='storage_locations')
  op.drop_index('idx_storage_locations_storage_area_id',
                table_name='storage_locations')
  ...
  op.execute(
      "DROP TRIGGER IF EXISTS trg_storage_location_containment ON storage_locations;")
  ...
  op.execute(
      "DROP TRIGGER IF EXISTS trg_storage_location_centroid ON storage_locations;")
