...
def upgrade() -> None:
  ...
  op.create_table(
      'storage_bins',
      sa.Column('bin_id', sa.Integer(), autoincrement=True, nullable=False,
                comment='Primary key (auto-increment)'),
      sa.Column('storage_location_id', sa.Integer(), nullable=False,
                comment='Parent storage location (CASCADE delete)'),
      sa.Column('storage_bin_type_id', sa.Integer(), nullable=True,
      ...
      sa.Column('status', postgresql.ENUM('active', 'maintenance', 'retired',
      ...
      sa.ForeignKeyConstraint(['storage_location_id'],
                              ...
                              name='fk_storage_bins_storage_location_id',
                              ondelete='CASCADE'),
      sa.ForeignKeyConstraint(['storage_bin_type_id'],
                              ['storage_bin_types.bin_type_id'],
                              name='fk_storage_bins_storage_bin_type_id',
                              ondelete='RESTRICT'),
      sa.UniqueConstraint('code', name='uq_storage_bins_code'),
      sa.CheckConstraint('LENGTH(code) >= 2 AND LENGTH(code) <= 100',
  ...
  op.create_index('idx_storage_bins_code', 'storage_bins', ['code'],
                  unique=True)
  op.create_index('idx_storage_bins_location_id', 'storage_bins',
                  ['storage_location_id'])
  op.create_index('idx_storage_bins_type_id', 'storage_bins',
                  ['storage_bin_type_id'])
