...
down_revision: Union[
  str, None] = '1wgcfiexamud'  # Fixed: comes after storage_bins
...
def upgrade() -> None:
  ...
  op.create_table(
      'product_states',
      sa.Column('product_state_id', sa.Integer(), autoincrement=True,
      ...
      sa.Column('is_sellable', sa.Boolean(), server_default='false',
                nullable=False,
  ...
  # Create indexes
  op.create_index('ix_product_states_code', 'product_states', ['code'],
                  unique=True)
  op.create_index('ix_product_states_is_sellable', 'product_states',
                  ['is_sellable'], unique=False)
  op.create_index('ix_product_states_sort_order', 'product_states',
  ...
  op.execute("""
             ...
             VALUES
                 ...
                 ('GERMINATING', 'Germinating',
                  'Seed has germinated, roots emerging', FALSE, 20),
                 ('SEEDLING', 'Seedling', 'Young plant with first true leaves',
                  FALSE, 30),
                 ('JUVENILE', 'Juvenile', 'Growing plant, not yet mature',
                  FALSE, 40),

                 -- MATURE STAGES (SELLABLE)
                 ('ADULT', 'Adult Plant', 'Mature plant, ready for sale', TRUE,
                  50),
                 ('FLOWERING', 'Flowering', 'Plant in bloom, highly desirable',
                  TRUE, 60),
                 ('FRUITING', 'Fruiting', 'Producing fruit/seeds', TRUE, 70),

                 -- SPECIAL STATES
                 ('DORMANT', 'Dormant',
                  'Resting period (winter dormancy, still sellable)', TRUE, 80),
                 ('PROPAGATING', 'Propagating',
                  'Cutting or division in rooting stage', FALSE, 90),

                 -- END-OF-LIFE (NOT SELLABLE)
                 ('DYING', 'Dying', 'Plant in decline, not recoverable', FALSE,
                  100),
                 ('DEAD', 'Dead', 'Plant is dead, awaiting disposal', FALSE,
                  110);
             """)
