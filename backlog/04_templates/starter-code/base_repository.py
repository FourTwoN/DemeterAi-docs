...
class AsyncRepository(Generic[ModelType]):
  ...
  async def get_multi(self, skip: int = 0, limit: int = 100, **filters) -> list[
    ModelType]:
    """
...
class StockMovementRepository(AsyncRepository["StockMovement"]):
  ...
  async def get_by_location(self, location_id: int, limit: int = 50) -> list[
    "StockMovement"]:
    """
    ...
    result = await self.session.execute(stmt)
    return list(
        result.unique().scalars().all())  # .unique() required with joinedload
