...
This diagram provides an **executive-level view** of the complete DemeterAI ML-powered plant
counting pipeline, from initial photo upload to final stock batch creation.
...
2. **API Layer**: FastAPI receives request, validates, generates UUIDs, saves temp files, and
...
3. **Celery Workers**: Three types of parallel workers:
    ...
    - **CPU workers** (pool=prefork): Aggregate results, create visualizations, generate stock
      batches
...
The diagram highlights the **critical path** from API entry through task dispatch, showing the
fastest route for request processing.
